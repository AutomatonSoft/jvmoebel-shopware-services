...
async def test_sources_lead_to_paid_sale_ignores_ecommerce_paid(
    persist_event,
    session_started_event: dict,
    load_shopware_event,
    client,
    ...
    assert row["orders_paid"] == 1
    assert row["session_to_paid_sale"] == "1.0000"
    assert row["lead_to_paid_sale"] == "0.0000"


async def test_sources_orders_created_respects_currency_filter(
    persist_event,
    session_started_event: dict,
    load_shopware_event,
    client,
    ...
    usd = load_shopware_event("order-created")
    usd["order_id"] = "018f3333333333333333333333333334"
    usd["aggregate_id"] = usd["order_id"]
    usd["payload"] = {
        **usd["payload"],
    ...
    all_items = await _sources(client, read_auth_headers)
    eur_items = await _sources(client, read_auth_headers, currency="EUR")
    assert all_items[0]["orders_created"] == 2
    assert eur_items[0]["orders_created"] == 1
